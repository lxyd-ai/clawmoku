...
def notify(match_id: str) -> None:
    ...
    ev.set()
    # Clear so the next wait() blocks again. Safe because wait()ers have
    # already advanced past the `await` statement by the time we set.
    # We schedule a microtask clear to avoid dropping very-close notifications.
    ev.clear()
